...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
     ...
     */
     ...
     // Base corner cases
     int minBudget = keyboards[0] + drives[0];
     if(minBudget > b){
         return -1;
     }
     
     // Base corner cases
     int maxValues = keyboards[keyboards.size() - 1] + drives[drives.size() - 1];
     if(maxValues <= b){
         return maxValues;
     }
     ...
     for(int i=keyboards.size() - 1; i>=0; i--){
         for(int j=drives.size() - 1; j>=0; j--){
             int currentBudget = keyboards[i] + drives[j];
             if(currentBudget > b){
                 continue;
             } else {
                 int diff = b - currentBudget;
