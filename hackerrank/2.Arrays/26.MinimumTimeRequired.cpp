...
long minTime(vector<long> machines, long goal) {
    long result = 0;
    long minDaysMachine = 1000000001;
    long maxDaysMachine = 0;
    long n = machines.size();
    maxDaysMachine = machines[machines.size() - 1];
    
    // Now calculate lower bound and upper bound
    long lowerBound, upperBound;
    lowerBound = 1;
    upperBound = maxDaysMachine * goal;
    
    while(lowerBound != upperBound){
        long middleBound = (lowerBound + upperBound)/2;
        
        long currentSum = 0;
        for(int i=0; i<machines.size(); i++){
            currentSum = currentSum + (middleBound/machines[i]);
        }
