...
Let's try to solve it with recursion.
Cool. When comparing last two digits, we just try to compare the values rather than creating a string and then converting it to int.
Once we have recursion working, we store the values of every index in our table.
*/
#include<bits/stdc++.h>
using namespace std;
...
int solve2(string &A, int indexA, unordered_map<int, int> &dict){
    if(indexA < 0) {
        return 1;
    }
    ...
    // if valid temp
    if(A[indexA] > '0') {
        result1 = solve2(A, indexA - 1, dict);
    }

    if(indexA > 0 && A[indexA - 1] == '1') {
        result2 = solve2(A, indexA - 2, dict);
    } else if(indexA > 0 && A[indexA - 1] == '2' && A[indexA] <= '6'){
        result2 = solve2(A, indexA - 2, dict);
    }

    result1 = result1 % MOD;
    result2 = result2 % MOD;
    long tempResult = result1 + result2;
    tempResult = tempResult % MOD;
    dict[indexA] = tempResult;
    return tempResult;
}
