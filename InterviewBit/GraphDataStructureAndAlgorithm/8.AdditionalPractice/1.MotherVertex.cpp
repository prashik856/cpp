...
2. If there exist mother vertex (or vertices), then v must be one (or one of them). 
...
class DisjointSet2 {
    ...
    public:
        ...
        int find(int node){
            int parentValue = parent[node];
            if(parentValue == node){
                return parentValue;
            } else {
                // We don't know parent, but we do know
                // that current parent of node is parentValue
                // so, we find the parent of parentValue
                parent[node] = find(parentValue);
            }
            return parent[node];
        }

        void dist_union(int parentNode, int childNode) {
            int parentOfParentNode = find(parentNode);
            int parentOfChildNode = find(childNode);
            if(parentOfParentNode == parentOfChildNode){
                return;
            }
            parent[parentOfChildNode] = parentOfParentNode;
        }
...
void runDFS(int root, vector< set<int> > &adj, vector<bool> &visited) {
    visited[root] = true;

    for(auto j=adj[root].begin(); j!=adj[root].end(); j++){
        int neighbour = *j;
        if(!visited[neighbour]){
            runDFS(neighbour, adj, visited);
        }
    }
}
...
int solve(int A, vector< vector<int> > &B){
    ...
    vector< set<int> > adj(A+1, set<int>());
    ...
    // store the last visited guy

    int v = 0;
    for(int i=1; i<=A; i++){
        if(!visited[i]){
            runDFS(i, adj, visited);
            v = i;
        }
    }

    // cout << "last visited vertex: " << v << endl;
    visited = vector<bool>(A+1, false);
    runDFS(v, adj, visited);
    for(int i=1; i<=A; i++){
        if(!visited[i]){
            result = 0;
        }
