...
*/
/*
We can just use custom travel.
keep track of a count, and for every count, visited as many nodes as possible using custom travel.
*/
#include<bits/stdc++.h>
using namespace std;
...
void custom_travel(int i, int j, vector<string> &a, vector< vector<bool> > &visited) {
    visited[i][j] = true;
    int row = i - 1; int col = j;
    if(row >=0 && row < a.size() && col >=0 && col < a[0].size()){
        if(a[row][col] == 'X' && !visited[row][col]){
            custom_travel(row, col, a, visited);
        }
    }
    row = i + 1; col = j;
    if(row >=0 && row < a.size() && col >=0 && col < a[0].size()){
